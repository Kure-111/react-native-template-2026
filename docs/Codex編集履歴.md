...
| 2026-03-08 | Codex | `src/features/item12/constants.js`<br>`src/features/item12/screens/Item12Screen.jsx`<br>`src/services/supabase/patrolTaskService.js`<br>`docs/アプリ理解.md`<br>`docs/管理部統合システム理解.md`<br>`docs/Codex編集履歴.md` | 巡回サポートから完了件数ランキング機能を削除し、タブ構成を `タスク` / `チェック` / `評価` / `履歴` / `企画一覧` に整理。あわせてランキング集計取得と表示コンポーネントを撤去 | `npm run build` を実行し、Web ビルド成功を確認 |
| 2026-03-08 | Codex | `src/features/item12/screens/Item12Screen.jsx`<br>`src/features/item12/components/PatrolTaskList.jsx`<br>`src/features/item12/components/PatrolTaskDetail.jsx`<br>`src/features/item12/components/PatrolCheckForm.jsx`<br>`src/features/item12/components/UnvisitedAlertList.jsx`<br>`src/features/item12/components/PatrolEvaluationForm.jsx`<br>`docs/アプリ理解.md`<br>`docs/管理部統合システム理解.md`<br>`docs/Codex編集履歴.md` | `frontend-design-ja` の方針を使い、巡回サポート UI を再設計。上部サマリーカード、大きめのタブカード、操作優先の詳細カード、巡回チェック/評価/未巡回アラートの視認性改善を行い、どのタブでも次アクションが先に分かる画面へ整理 | `npm run build` を実行し、Web ビルド成功を確認 |
| 2026-03-08 | Codex | `src/features/item12/constants.js`<br>`src/features/item12/screens/Item12Screen.jsx`<br>`docs/アプリ理解.md`<br>`docs/管理部統合システム理解.md`<br>`docs/Codex編集履歴.md` | 巡回サポートの見た目を再整理し、下部タブを `タスク` / `チェック` / `企画一覧` / `ダッシュボード` の4つへ再編。各タブに繰り返し出していた上部ダッシュボード風 UI を撤去し、要約情報は `ダッシュボード` タブだけに集約した | ユーザー指摘の「見づらい」「各画面にダッシュボードがあって邪魔」を受けて構成を簡素化。`npm run build` 成功 |
| 2026-03-08 | Codex | `src/services/supabase/patrolCheckService.js`<br>`src/features/item12/screens/Item12Screen.jsx`<br>`src/features/item12/components/PatrolCheckForm.jsx`<br>`docs/アプリ理解.md`<br>`docs/管理部統合システム理解.md`<br>`docs/Codex編集履歴.md` | 定常巡回チェックを `organizations_events` ベースへ切り替え、`organization_name` 順の企画候補から1件選択して登録する形に変更。あわせて各チェック項目を5段階評価 + 項目別メモで入力できるようにし、`patrol_checks.check_items` も JSON オブジェクト配列を扱えるよう拡張 | 旧来の文字列配列形式も履歴表示では後方互換で読めるようにし、`npm run build` 成功を確認 |
| 2026-03-08 | Codex | `src/features/support/components/SupportDeskScreen.jsx`<br>`docs/アプリ理解.md`<br>`docs/管理部統合システム理解.md`<br>`docs/Codex編集履歴.md` | 本部サポートの概況タブで `企画報告確認` と `施錠確認` を個別に折りたためるようにし、`施錠確認` は `担当あり` / `担当なし` と `確認済み` / `未確認` の条件で絞り込めるようにした | `npm run build` を実行し、Web ビルド成功を確認 |
